...
function Sidebar() {
  ...
  return (
    ...
    <Flex direction={"column"} gap={10} w={"full"} height={"full"}>
      ...
      <Link top={"/"} as={RouterLink} p={2} display={{base:"block", md:"none"}} borderRadius={6} _hover={{bg: "whiteAlpha.200"}} w={10} cursor="pointer">
      <InstagramMobileLogo />
      </Link>
      ...
      <Tooltip hasArrow label={"Logout"} placement='right' ml={1} openDelay={500} display={{base:"block" , md:"none"}}>
            ...
            >
              <BiLogOut size={25}/>
              <Button variant={"ghost"} _hover={{bg:"transparent"}} display={{base:"none", md:"block"}} isLoading = {isLoggingOut} >
                Logout
              </Button>
            </Flex>
          </Tooltip>
